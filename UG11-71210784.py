class RakObat:
    ...
    def ambilObat(self, jnsObat):
        key_hash = self._getHash(jnsObat)
        if self.map[key_hash] is None:
            return False
        for index in range(self.size):
           
            key_hash = self._linearProbing(jnsObat, index)
           
            if(self.map[key_hash][0][0] == jnsObat):
                print("obat {} diambil dari rak" .format(jnsObat))
                self.map[key_hash] = "deleted"
                return True

        print("Key ", jnsObat, " tidak ditemukan")
        return False
